const grades = [
    ...
    function renderQOTD() {
      ...
      const boost = document.getElementById('brainBoostBtn');
      ...
      const toggle = () => {
        ...
        if (boost) boost.textContent = isHidden ? 'Daily brain boost' : 'Hide answer';
        if (btn) btn.setAttribute('aria-expanded', String(!isHidden));
        if (boost) boost.setAttribute('aria-expanded', String(!isHidden));
      };
      if (ans) {
        ans.textContent = today.answer;
      ...
      if (boost) {
        boost.textContent = 'Daily brain boost';
        boost.setAttribute('aria-expanded', 'false');
        boost.onclick = toggle;
      }
    }
    ...
    function answer(choice) {
      ...
      fb.innerHTML = ok
        ? `Correct! 🎉 <br><span class="small">${q.explain}</span>`
        : `Almost there. Correct answer: <b>${label}</b>. <br><span class="small">${q.explain}</span>`;
      ...
      clearTimeout(state.transitionTimeout);
      const isLastQuestion = state.index >= state.list.length - 1;
    ...
    function resetQuizBoard() {
      ...
      const subtitle = document.getElementById('quizSubtitle'); if (subtitle) subtitle.textContent = 'Select a grade card to begin. Answers shuffle each time, and challenge questions get tougher as you progress.';
    ...
    function saveLogin() {
      ...
      if (line) line.textContent = name ? `Welcome back, ${name}! ${level ? 'Level: ' + level + '.' : ''}` : '';
      updateProfileStatus();
      closeLogin();
    }
    ...
    function init() {
      ...
      document.getElementById('quizSubtitle').textContent = 'Select a grade card to begin. Answers shuffle each time, and challenge questions get tougher as you progress.';
      document.getElementById('timeTxt').textContent = '--';
      document.getElementById('clock').style.setProperty('--pct', '0deg');
      if (state.userName) {
        const line = document.getElementById('welcomeLine');
        if (line) line.textContent = `Welcome back, ${state.userName}! ${state.userLevel ? 'Level: ' + state.userLevel + '.' : ''}`;
      }
